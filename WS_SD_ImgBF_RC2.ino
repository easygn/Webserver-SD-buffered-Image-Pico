...
#define F_BUF_SZ    1024  // 16384 +0.5s  4096,8192 +0.1~.2s  overhead canceled   
                ...
                if (c == '\n' && currentLineIsBlank) {
                    ...
                    if (webFile) {                    
                        ...
                        // unsigned short int WBCur, freeWzln;
                                                                    //  Serial.print("File Open Before memory : ");
                                                                    //  Serial.println(rp2040.getFreeHeap());         // SRAM check - Start
                        while(freeFiln) {
                          if (freeFiln > F_BUF_SZ) freeFiln = F_BUF_SZ;
                          webFile.read(fBuf, freeFiln);
                                                                    //  Serial.print("File Open After memory : ");
                                                                    //  Serial.println(rp2040.getFreeHeap());         // SRAM check - End
/*
                          WBCur = 0;
                          while (WBCur > WBCur) {
